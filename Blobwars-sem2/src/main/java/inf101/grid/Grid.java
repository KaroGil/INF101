...
public class Grid<T> implements IGrid<T> {
	...
	/**
	 * method to get all the surrounding blocks within a 2 block radius
	 * @param loc the center location of the neighbourhood
	 * @param distance 1 or 2 depending on if its the closest neighbour or the 5x5 grid
	 * @return list of surrounding location
	 */
	public List<Location> getNeighbourhood(Location loc, int distance){
		List<Location> moveOneList = new ArrayList<Location>();
        for (GridDirection dir : GridDirection.EIGHT_DIRECTIONS) {
			if(isOnGrid(loc.getNeighbor(dir))){
				moveOneList.add(loc.getNeighbor(dir));
			}
        }
        if(distance == 2){
            List<Location> move2List = new ArrayList<>();
            for (Location location : moveOneList) {
				for(Location loc2 : location.allNeighbors()){
					if(isOnGrid(loc2)){
						move2List.add(loc2);
					}
				}
