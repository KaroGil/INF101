...
public class GameGUI extends Input implements ActionListener, Graphics {
	...
	/**
	 * Waits for 2 gamepanels to be selected then gets the gamepanel list and uses  
	 * this to get one location from and one location to.
	 * @return Move with the locFrom and locTo values.
	 */
	@Override
	public Move getMovement() {
		List<Location> selectedPanels = getSelectedPanels(2);
		Location locFrom = selectedPanels.get(0);
		Location locTo = selectedPanels.get(1);
		return new Move(locFrom, locTo);
	}
